...
if (!empty($form_data->barcode_scantrans)) {
    $barcode_scantrans = mysqli_real_escape_string($dbcon, $form_data->barcode_scantrans);
...
if (empty($error1) && empty($error2) && empty($error3) && empty($error4)) {
    $barcode_scantrans = mysqli_real_escape_string($dbcon, $form_data->barcode_scantrans);
    $input_Dienstleister = mysqli_real_escape_string($dbcon, $form_data->input_Dienstleister);
    if (empty($form_data->input_Tracking)) {
        $input_Tracking = "";
    }
    if (!empty($form_data->input_Tracking)) {
        $input_Tracking = mysqli_real_escape_string($dbcon, $form_data->input_Tracking);
        if (($form_data->input_Dienstleister == "TNT") && (strlen($form_data->input_Tracking) == 28)) {
            $input_Tracking = substr($input_Tracking, 4, -15);
        }
        if (($form_data->input_Dienstleister == "TNT") && (strlen($form_data->input_Tracking) == 24)) {
            $input_Tracking = substr($input_Tracking, 0, -15);
        }
    }

    $input_Transbearbeiter = "1"; //mysqli_real_escape_string($dbcon, $form_data->input_transbearbeiter);
    $datum = date("d.m.Y");
    $uhrzeit = date("H:i:s");
    $statetrans = "1";
    $stateklaer = "";
