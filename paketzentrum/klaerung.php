...
<div class="form-group center-block text-center col-md-6">
        ...
        <option ng-click="insertData()" value="Greenlight">Greenlight</option>
        <option ng-click="insertData()" value="Greenlight STPI">Greenlight STPI</option>
        <option ng-click="insertData()" value="Ansprechpartner fehlt">Ansprechpartner fehlt</option>
        <option ng-click="insertData()" value="DHL/TNT Account Nr. fehlt">DHL/TNT Account Nr. fehlt</option>
      </datalist>
...
<!-- Bearbeiter -->
     ...
     <a href='klaerungedit.php'><button type='button' style='position: absolute; right: 235;'class='btn btn-dark rb-button-sm'>Klaerungsgrund ändern</button></a>
     </div>    
    </form>
   </div>
  </div>
...
<div class="table-responsive border bg-light"> <!--this is used for responsive display in mobile and other devices-->
    <table id="Arbeitsvorrat" class="table table-bordered table-hover table-striped" style="table-layout: fluid">  
        ...
        if($buero == 1 || $Admin == 1 ) {
            $view_arbeitsvorrat_query="select * FROM versendungen WHERE Status_Klaerung='1' && Status_Storno='' ORDER by Prioritaet_Wareneingang='eilig' DESC, STR_TO_DATE(Datum_Klaerung, '%d.%m.%Y'), Zeit_Klaerung ASC";
        } else {
            //$view_arbeitsvorrat_query="select * FROM versendungen WHERE Status_Klaerung='1' && Status_Storno='' && Bearbeiter_VKS_Klaerung!='57' && Bearbeiter_VKS_Klaerung!='66' && Bearbeiter_VKS_Klaerung!='69' && Bearbeiter_VKS_Klaerung!='77' && Bearbeiter_VKS_Klaerung!='99' ORDER by Prioritaet_Wareneingang='eilig' DESC, STR_TO_DATE(Datum_Klaerung, '%d.%m.%Y'), Zeit_Klaerung ASC";
            $view_arbeitsvorrat_query="select * FROM versendungen WHERE Status_Klaerung='1' && Status_Storno='' && Weitere_Bearbeitung!='Versandbüro' ORDER by Prioritaet_Wareneingang='eilig' DESC, STR_TO_DATE(Datum_Klaerung, '%d.%m.%Y'), Zeit_Klaerung ASC";
