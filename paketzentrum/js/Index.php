...
$pdo = new PDO('mysql:host=127.0.0.1;dbname=paketzentrum', 'root', '');

$searchfor = $_POST['suchbegriff'];

$dateklaerung = $pdo->prepare("SELECT TIMESTAMPDIFF(SECOND, STR_TO_DATE(CONCAT(Datum_Klaerung, Zeit_Klaerung), '%d.%m.%Y%H:%i:%s'), STR_TO_DATE(CONCAT(Datum_Wareneingang, Zeit_Wareneingang), '%d.%m.%Y%H:%i:%s'))/86400 AS timediffw, TIMESTAMPDIFF(SECOND, STR_TO_DATE(CONCAT(Datum_Klaerung, Zeit_Klaerung), '%d.%m.%Y%H:%i:%s'), STR_TO_DATE(CONCAT(Datum_verpacken, Zeit_verpacken), '%d.%m.%Y%H:%i:%s'))/86400 AS timediffv, TIMESTAMPDIFF(SECOND, STR_TO_DATE(CONCAT(Datum_Klaerung, Zeit_Klaerung), '%d.%m.%Y%H:%i:%s'), STR_TO_DATE(CONCAT(Datum_Bereitstellung, Zeit_Bereitstellung), '%d.%m.%Y%H:%i:%s'))/86400 AS timediffb, TIMESTAMPDIFF(SECOND, STR_TO_DATE(CONCAT(Datum_Klaerung, Zeit_Klaerung), '%d.%m.%Y%H:%i:%s'), STR_TO_DATE(CONCAT(Datum_Transportpapiere, Zeit_Transportpapiere), '%d.%m.%Y%H:%i:%s'))/86400 AS timedifft, TIMESTAMPDIFF(SECOND, STR_TO_DATE(CONCAT(Datum_Klaerung, Zeit_Klaerung), '%d.%m.%Y%H:%i:%s'), STR_TO_DATE(CONCAT(Datum_Versendung, Zeit_Versendung), '%d.%m.%Y%H:%i:%s'))/86400 AS timediffversa FROM versendungen WHERE Datum_Klaerung NOT LIKE '' AND Status_Klaerung NOT LIKE '1' AND referenznr = :referenznr");
$dateklaerung->execute(array('referenznr' => "$searchfor"));
$result = $dateklaerung->fetch(PDO::FETCH_ASSOC);
	
$statement = $pdo->prepare("SELECT * FROM versendungen WHERE referenznr LIKE :referenznr OR Lieferschein = :Lieferschein LIMIT 30");

if($statement->execute(array('referenznr' => "%$searchfor%", 'Lieferschein' => "$searchfor"))) {
...
if($Storno == 1) {
  ...
  if($Klaerung == 1) {
    ...
    if($ZeitKL > 0 && $Klaerung == 0) {
      echo "<h5><img src='view/media/warning-yellow.svg' width='50' height='50' alt=''><b class='bg-warning'>&nbsp &nbsp Der Auftrag war am: $DatumKL  /  $ZeitKL  für $klaerzeit Tage in Klärung &nbsp - &nbsp Grund: $Klaerungsgrund &nbsp - &nbsp Bearbeiter: $Bearbeiterklaerung &nbsp &nbsp";
      echo"</h5></b>";
    }
...
echo "<u>Priorität:</u> $PrioW<br><br>";
