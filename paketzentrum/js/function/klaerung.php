...
if(!empty($form_data->barcode_scanklaer))
{
    $barcode_scanklaer = mysqli_real_escape_string($connect, $form_data->barcode_scanklaer);
...
if(empty($error1) && empty($error2) && empty($error3))
{
 ...
 $input_Klaerbearbeiter = mysqli_real_escape_string($connect, $form_data->input_Klaerbearbeiter);
 $datum = date("d.m.Y");
 $uhrzeit = date("H:i:s");
 $stateklaer = "1";
